...
# Already done via the custom vm-image scripts
...
if [ "${SHARED_STORAGE_ACCESS}" -eq 1 ]; then
  ...
  # Configure autofs
  # echo "$fileShareName -fstype=cifs,rw,dir_mode=0777,credentials=$smbCredentialFile, mfsymlinks :$smbPath" | sudo tee /etc/auto.fileshares > /dev/null
